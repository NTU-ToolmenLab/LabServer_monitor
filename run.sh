...
docker run --rm -it -v "$PWD:/app" dcagatay/j2cli k8s/grafana.yml config.yaml -o k8s/grafana.yml
docker run --rm -it -v "$PWD:/app" dcagatay/j2cli k8s/monitor.yml config.yaml -o k8s/monitor.yml
